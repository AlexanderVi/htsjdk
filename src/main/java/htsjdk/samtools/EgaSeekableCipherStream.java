 ...
 * @author asenf
 */
public class EgaSeekableCipherStream extends SeekableStream {
    ...
    // Constructors
    public EgaSeekableCipherStream(SeekableStream in, char[] password) {
        this(in, password, DEFAULT_BUFFER_SIZE);
    }
    public EgaSeekableCipherStream(SeekableStream in, char[] password, int bufferSize) {
        this(in, password, bufferSize, 128);
    }
    public EgaSeekableCipherStream(SeekableStream in, char[] password, int bufferSize, int pw_strength) {
        ...
        byte[] iv = new byte[16];
        try { in.read(iv, 0, 16); } catch (IOException ex) {}
        ...
        this.cipher = EgaCipherStream.getCipher(password, false, pw_strength, this.orig_digest);
    ...
    public void seek(long position) throws IOException {
        ...
        try {
            ...
            Logger.getLogger(EgaSeekableCipherStream.class.getName()).log(Level.SEVERE, null, ex);
        }
        this.incipher = new CipherInputStream(this.wrappedStream, cipher);
    }
